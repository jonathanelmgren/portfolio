...
export const generateLogReport = (data: TDriversLogGenerator): TDriversLogReport => {
    ...
    for (const randomTrip of randomTrips) {
        ...
        const amountOfTripsNeeded = Math.ceil(availableMileagesForThisTrip / tripInKm);
        ...
        if (roundTrip && amountOfTripsNeeded > availableDays.length * 2) throw new Error('Not enough days to cover all trips, consider lowering amount of work trips')
        if (!roundTrip && amountOfTripsNeeded > availableDays.length) throw new Error('Not enough days to cover all trips, consider lowering amount of work trips');

        let addedTrip = 0;
        let drivenThisTrip = 0;
        while (availableDays.length > 0 && addedTrip <= amountOfTripsNeeded && availableMileagesForThisTrip > (drivenThisTrip + tripInKm)) {
    ...
    const sortedTripsByDate = trips.sort((a, b) => a.date.getDate() - b.date.getDate())
...
function weightedRandom(min: number, max: number) {
    ...
    return Math.round(Math.random() * (selectedRange.to - selectedRange.from) + selectedRange.from);
}
